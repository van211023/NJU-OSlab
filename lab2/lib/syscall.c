...
char getChar(){ // 对应SYS_READ STD_IN
	...
	char ch = 0;
	return syscall(SYS_READ, STD_IN, (uint32_t)(ch), 1, 0, 0);
	//return ch;
}
...
void getStr(char *str, int size){ // 对应SYS_READ STD_STR
	...
	//	str[i] = 0;
	syscall(SYS_READ, STD_STR, (uint32_t)str, size - 1, 0, 0);
	str[size - 1] = '\0';
}
	...
	while(format[i]!=0){
		...
		if(format[i]=='%' && format[i + 1] != 0){
			...
			case 'd':
				...
				//decimal = ((char **)paraList)[index];
				count = dec2Str(decimal, buffer, MAX_BUFFER_SIZE, count);

				break;
			case 'x':
				index++;
				hexadecimal = va_arg(ap, unsigned);
			...
			case 's':
				...
				//string = (uint32_t*)paraList[index];
				count = str2Str(string, buffer, MAX_BUFFER_SIZE, count);	
				break;
			case 'c':
				index++;				
				character = va_arg(ap, unsigned);
				//character = string[0];
				//character = (uint32_t*)paraList[index];
